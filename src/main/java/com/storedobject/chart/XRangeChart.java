 ...
 * {@link RectangularCoordinate} system a range of values on the X-axis with corresponding Y-axis values. Each X
 * range-value is represented using a horizontal bar. Typically, X-values are of {@link DataType#NUMBER},
 * {@link DataType#DATE} or {@link DataType#TIME} and Y-values are of {@link DataType#CATEGORY}. However, the Y-values
 * could be of other types too.
 * <p>
...
public class XRangeChart<X, Y> implements ComponentGroup {
	 ...
	 *
	 * @param xLowerValue
	 *            Lower value of the X-range.
	 * @param xUpperValue
	 *            Upper value of the X-range.
	 * @param yValue
	 *            Y-value.
	 * @param label
	 *            A label to be rendered on the bar. (Could be null or empty string if you don't want a label).
	 * @param color
	 *            Color of the bar.
	 */
	public void addData(final X xLowerValue, final X xUpperValue, final Y yValue, final String label,
			final Color color) {
	 ...
	 */
	public void addData(final X xLowerValue, final X xUpperValue, final Y yValue, final String label, final Color color,
	 ...
	 */
	public void addData(final X xLowerValue, final X xUpperValue, final Y yValue, final Color color,
	 ...
	 */
	public void addData(final X xLowerValue, final X xUpperValue, final Y yValue, final Color color,
			final double progressPercentage, final String extraInfo) {
	...
	@Override
	public void removeParts(final SOChart soChart) {
	 ...
	 * @return Zoom data.
	 */
	public DataZoom getXZoom(final boolean create) {
	 ...
	 * @return Zoom data.
	 */
	public DataZoom getYZoom(final boolean create) {
	 ...
	 *            True/false.
	 */
	public void showProgressLabel(final boolean showProgressLabel) {
	 ...
	 *            Y-data.
	 */
	public void setYData(final CategoryDataProvider yData) {
	...
	public RectangularCoordinate getCoordinateSystem() {
		if (coordinate == null) {
			...
			final Position p = coordinate.getPosition(true);
	...
	/**
	 * Format a Y-value to show as part of the tooltip. (A null may be returned to ignore the value). By default, it
	 * invokes {@link #formatValue(Object)} and adds a colon followed by a space.
	 *
	 * @param value
	 *            Value to format.
	 * @return Formatted value.
	...
	/**
	 * Format an X-value to show as part of the tooltip. (A null may be returned to ignore the value). By default, it
	 * invokes {@link #formatValue(Object)}.
	 *
	 * @param value
	 *            Value to format.
	 * @return Formatted value.
	 */
	public String formatXValue(final X value) {
		return formatValue(value);
	}

	/**
	 * Format the X-values to show as part of the tooltip. (A null may be returned to ignore the value). By default, it
	 * invokes {@link #formatXValue(Object)} for each of the values and concatenate it with a dash (" - ") as the
	 * delimiter.
	 *
	 * @param xLowerValue
	 *            Lower value of the X-range.
	 ...
	 * @return Formatted value.
	 */
	public String formatXValue(final X xLowerValue, final X xUpperValue) {
		final String s = formatXValue(xLowerValue);
		if (s == null) {
			return null;
		}
	 ...
	 * @return Formatted value.
	 */
	public String getTooltipText(final X xLowerValue, final X xUpperValue, final Y yValue,
			@SuppressWarnings("unused") final String label, @SuppressWarnings("unused") final double progress,
			final String extraInfo) {
		String sb = "";
	...
	private class Data {
		...
		private Data(final X xLowerValue, final X xUpperValue, final Y yValue, final String label, final Color color,
				final double progress, final String extraInfo) {
		...
		protected AbstractDataProvider<?> dataToEmbed() {
			if (data == null) {
				...
				data = new BasicDataProvider<String>() {
					...
					@Override
					public Stream<String> stream() {
						return XRangeChart.this.data.stream().map(d -> "[" + yAxis.getDataType().encode(d.yValue)
								+ ",\"" + d.label() + "\"," + xAxis.getDataType().encode(d.xLowerValue) + ","
								+ xAxis.getDataType().encode(d.xUpperValue) + "," + (progress ? d.progress : 100) + ","
								+ d.color + "," + d.color + "," + (progress ? "\"black\"" : d.color) + "]");
					}
