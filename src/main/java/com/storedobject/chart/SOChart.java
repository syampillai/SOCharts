 ...
 * {@link ComponentPart}s. Examples of chart {@link Component}s are (a) {@link Chart}, (b)
 * {@link RectangularCoordinate}, (c) {@link PolarCoordinate} etc. An example of a {@link ComponentPart} is
 * {@link AngleAxis} that is used by the {@link PolarCoordinate}.
 * </p>
 * <p>
...
public class SOChart extends LitComponent implements HasSize {
	...
	final static ComponentEncoder[] encoders = { new ComponentEncoder("*", DefaultColors.class),
			...
			new ComponentEncoder(Legend.class), new ComponentEncoder(Tooltip.class),
			...
			new ComponentEncoder("polar", PolarCoordinate.class), new ComponentEncoder("radar", RadarCoordinate.class),
			new ComponentEncoder("grid", RectangularCoordinate.class), new ComponentEncoder("series", Chart.class),
			new ComponentEncoder("dataZoom", AbstractDataZoom.class),
			new ComponentEncoder("visualMap", VisualMap.class), new ComponentEncoder("graphic", Shape.class),
	 ...
	 *            Background color.
	 */
	public void setDefaultBackground(final Color background) {
		defaultBackground = background;
	}

	/**
	 * Get the default text style. You may invoke this method and override default values if required. However, please
	 * note that setting padding, border (not text border) and alignment properties do not have any effect.
	 *
	...
	/**
	 * A tooltip will be shown by default. However, you can either disable it using this method or you can create your
	 * own customized tooltips and add it using {@link #add(Component...)}.
	 */
	...
	/**
	 * Legends will be shown by default. However, you can either disable it using this method or you can create your own
	 * customized legends and add it using {@link #add(Component...)}.
	 */
	...
	/**
	 * This should be invoked only from {@link Component#addParts(SOChart)} method (That method will be called on each
	 * {@link Component} whenever chart is getting updated).
	 *
	 * @param parts
	 *            Parts to add.
	 */
	void addParts(final ComponentPart... parts) {
		if (parts != null) {
			for (final ComponentPart cp : parts) {
				if (cp != null) {
					this.parts.add(cp);
				}
			}
	 ...
	 *            Data to add.
	 */
	public void addData(final AbstractDataProvider<?> data) {
	 ...
	 *            Data to remove.
	 */
	public void removeData(final AbstractDataProvider<?> data) {
	 ...
	 */
	public void add(final ComponentGroup... componentGroups) {
		if (componentGroups != null) {
			for (final ComponentGroup componentGroup : componentGroups) {
	 ...
	 */
	public void remove(final ComponentGroup... componentGroups) {
		if (componentGroups != null) {
			for (final ComponentGroup componentGroup : componentGroups) {
				if (componentGroup != null) {
	...
	/**
	 * Clear the chart. This will remove the chart display. However, it can be rendered again by invoking
	 * {@link #update()} as long as {@link #removeAll()} is not called.
	 */
	public void clear() {
		if (neverUpdated) {
			return;
	 ...
	 * client-side. So, it is useful only if it is a partial update. Old set of data passed will be used for the display
	 ...
	 * </p>
	 * <p>
	 * Even after eliminating the overhead of data, we can eliminate other components that are not changed via the
	 * method {@link #remove(Component...)}.
	 * </p>
	 *
	 * @param skipData
	...
	public void update(boolean skipData) throws ChartException, Exception {
		...
		for (final ComponentGroup cg : componentGroups) {
			cg.validate();
			cg.addParts(this);
		}
		...
		final List<AbstractDataProvider<?>> data = new ArrayList<>();
		dataSet.clear();
		...
		final Set<AbstractDataProvider<?>> collectData = new HashSet<>();
		...
		while (!data.isEmpty()) {
			final AbstractDataProvider<?> d = data.remove(0);
	 ...
	 * </p>
	 *
	 * @param componentPartClass
	 *            The component part class that is currently added. You may add more of these.
	 * @param buffer
	 *            The buffer that contains the code that is already generated.
	 */
	@SuppressWarnings("unused")
	protected void addCustomEncoding(final Class<? extends ComponentPart> componentPartClass,
			final StringBuilder buffer) {
	}

	/**
	 * This method is invoked after rendering each {@link ComponentPart}. You can add custom code into the buffer that
	 * already contains the encoding for the part. Also, you can add at the root-level (not related to any particular
	 ...
	 *
	 * @param componentPart
	 *            The component part to which custom code can be added. If this is null, then code added will go to the
	 *            root-level.
	 * @param buffer
	 *            The buffer that contains the code that is already generated.
	 */
	@SuppressWarnings("unused")
	protected void addCustomEncoding(final ComponentPart componentPart, final StringBuilder buffer) {
	}

	/**
	 * This method is invoked just before the JSON string that is being constructed in the {@link #update()} method is
	 * sent to the client. The returned value by this method will be sent to the "echarts" instance at the client-side.
	 * The default implementation just returns the same string. However, if someone wants to do some cutting-edge
	 * customization, this method can be used. This JSON string is used to construct the "option" parameter for the
	 * "echarts.setOption(option)" JavaScript method.
	 *
	 * @param json
	 *            JSON string constructed by the {@link #update()} method.
	 * @return Customized JSON string.
	 * @throws Exception
	 *             If any custom error to be notified so that rendering will not happen.
	 */
	@SuppressWarnings("RedundantThrows")
	protected String customizeJSON(final String json) throws Exception {
		return json;
	}

	/**
	 * This method is invoked just before the JSON string that is being constructed to send data to the client. The
	 * returned value by this method will be sent to the "echarts" instance at the client-side. The default
	 * implementation just returns the same string. However, if someone wants to do some cutting-edge customization,
	 * this method can be used.
	 *
	 * @param json
	 *            JSON string constructed by the {@link #update()} method.
	 * @param data
	 *            The data part for which this JSON string was created.
	 * @return Customized JSON string.
	 * @throws Exception
	 *             If any custom error to be notified so that rendering will not happen.
	 */
	@SuppressWarnings("RedundantThrows")
	protected String customizeDataJSON(final String json,
			@SuppressWarnings("unused") final AbstractDataProvider<?> data) throws Exception {
		return json;
	}

	private void initData(final AbstractDataProvider<?> data) throws Exception {
	 ...
	 *            Data to be updated.
	 * @throws Exception
	 *             if custom error is raised by {@link #customizeDataJSON(String, AbstractDataProvider)}.
	 */
	public void updateData(final AbstractDataProvider<?>... data) throws Exception {
		if (data != null) {
			for (final AbstractDataProvider<?> d : data) {
				if (d != null) {
					if (d.getSerial() > 0) {
						updateData("update", d);
					}
	 ...
	 *
	 * @param dataOwners
	 *            Data owners whose data to be updated.
	 * @throws Exception
	 *             if custom error is raised by {@link #customizeDataJSON(String, AbstractDataProvider)}.
	 */
	public void updateData(final HasData... dataOwners) throws Exception {
		if (dataOwners != null) {
			final Set<AbstractDataProvider<?>> dataSet = new HashSet<>();
			for (final HasData hd : dataOwners) {
				if (hd != null) {
	...
	static class ComponentEncoder {
		...
		private ComponentEncoder(final Class<? extends ComponentPart> partType) {
			this(null, partType);
		}

		private ComponentEncoder(String label, final Class<? extends ComponentPart> partType) {
		...
		private void encode(final SOChart soChart, final StringBuilder sb,
				final List<? extends ComponentPart> components) {
			int renderingIndex = 0;
			int serial = -2;
			for (final ComponentPart c : components) {
	 ...
	 * @return Color.
	 */
	public static Color getDefaultColor(final int index) {
	...
	private static class DefaultColors extends ArrayList<AbstractColor> implements ComponentPart {
		...
		public void encodeJSON(final StringBuilder sb) {
			...
			int count = 0;
			boolean first = true;
			for (final AbstractColor c : this) {
				if (c == null) {
					continue;
				}
				if (first) {
	...
	private static class DefaultTextStyle implements ComponentPart {
		...
		@Override
		public void encodeJSON(final StringBuilder sb) {
			final TextStyle.OuterProperties op = new TextStyle.OuterProperties();
			textStyle.save(op);
			sb.append("\"textStyle\":{");
