 ...
 * {@link #setType(ChartType)} method. However, there are concrete derivatives of this class such as {@link PieChart},
 * {@link NightingaleRoseChart} etc. where more chart-specific methods are available and data for the chart is checked
 * more accurately for errors. If the data set for the chart is of invalid type, system tries to do its best to adapt
 * that data but the chart may not appear if the data conversion fails.
 * </p>
 * <p>
 * Custom Charts: Any chart may be converted to a custom chart by setting up an item renderer to render the each data
 * point. The item renderer can be specified via {@link #setCustomRenderer(String)}. However, please note that only a
 * few custom renderers are currently available.
 * </p>
 * <p>
 * Positioning of charts within the display area of {@link SOChart}: Most charts need a {@link CoordinateSystem} to plot
 * on and all {@link CoordinateSystem}s support positioning (Please see {@link CoordinateSystem#setPosition(Position)}).
 * Those which do not require a {@link CoordinateSystem} are called {@link SelfPositioningChart} and supports its own
 * positioning mechanism (Please see {@link SelfPositioningChart#setPosition(Position)}).
 * </p>
 *
 * @author Syam
 */
public class Chart extends AbstractPart implements Component, HasData, HasAnimation, HasEmphasis {
	...
	public void addEvent(final SOEvent key, final Runnable value) {
	 ...
	 */
	public Chart(final AbstractDataProvider<?>... data) {
		this(null, data);
	}
	 ...
	 *            Data to be used (multiples of them for charts that use multi-axis coordinate systems).
	 */
	public Chart(final ChartType type, final AbstractDataProvider<?>... data) {
		setType(type);
		this.data = data;
	}
	 ...
	 *            Data to be used (multiples of them for charts that use multi-axis coordinate systems).
	 */
	public void setData(final AbstractDataProvider<?>... data) {
		this.data = data;
	}
	...
	/**
	 * <p>
	 * Set a custom renderer for this chart. Each renderer can be used on certain type of charts only, may require
	 ...
	 */
	public void setCustomRenderer(final String renderer) {
		custom = renderer;
	}
	 ...
	 * than pointing to the dataset. Example: {@link TreeChart}, {@link GaugeChart} etc. If null is returned from this
	 * method, no data will be embedded.
	 *
	 ...
	 * The default implementation returns the result of {@link #dataToEmbed()} if it is not null. Otherwise, it tries to
	 * determine it from the axis data if possible.
	 *
	...
	/**
	 * Get the index to get the real data value of this chart. (In special charts, the actual data value at a data point
	 * may at an index different from 0).
	 *
	...
	public void encodeJSON(final StringBuilder sb) {
		...
		final AbstractDataProvider<?> dataToEmbed = dataToEmbed();
		if (dataToEmbed != null) {
			...
			if (coordinateSystem.axes != axes) {
				...
				axisClasses.forEach(ac -> coordinateSystem.axes(ac).map(a -> a.wrap(coordinateSystem))
						.min(Comparator.comparing(ComponentPart::getSerial)).ifPresent(w -> {
							final Axis.AxisWrapper aw = (Axis.AxisWrapper) w;
	...
	public void validate() throws ChartException {
		final String[] axes = type.getAxes();
		if (data == null) {
	 ...
	 * <p>
	 * Note: Some charts like {@link PieChart}, {@link RadarChart} etc. do not have a coordinate system.
	 * </p>
	 *
	...
	private String getPropertyName(final ComponentProperty property) {
	...
	<P extends ComponentProperty> void setPropertyName(final Class<P> propertyClass, final String name) {
		propertyNameMap.put(propertyClass, name);
	}

	<P extends ComponentProperty> P getProperty(final Class<P> propertyClass, final boolean create) {
	...
	void setProperty(final ComponentProperty property) {
		propertyMap.put(property.getClass(), property);
	}

	String axisName(final int axis) {
	...
	public void addParts(final SOChart soChart) {
		...
		final Tooltip tooltip = getTooltip(false);
		if (tooltip != null) {
	...
	@Override
	public void declareData(final Set<AbstractDataProvider<?>> dataSet) {
		dataSet.addAll(Arrays.asList(data));
		final AbstractDataProvider<?> d = dataToEmbed();
		if (d != null) {
			dataSet.add(d);
		}
		if (markArea != null) {
	...
	/**
	 * Plot the chart on a given coordinate system. (Certain chart types such as {@link ChartType#Pie}, do not have a
	 * coordinate system and thus, this call is not required. Also, instead of using this method, you can use the
	 * {@link CoordinateSystem#add(Chart...)} method if you want to plot on the default set of axes.)
	 *
	 * @param coordinateSystem
	 *            Coordinate system on which the chart will be plotted. (If it was plotted on another coordinate system,
	 *            it will be removed from it).
	 * @param axes
	 *            Axes to be used by the chart. This needs to be specified if the coordinate system has multiple axes of
	 *            the required type.
	 * @return Self reference.
	 */
	public Chart plotOn(final CoordinateSystem coordinateSystem, final Axis... axes) {
		if (coordinateSystem == null) {
			this.axes = null;
	 ...
	 *
	 * @param axes
	 *            Axes to be used by the chart.
	 * @return Self reference.
	 */
	public Chart plotOn(final Axis... axes) {
	...
	/**
	 * Set colors for the charts. Certain charts require more than one color, e.g., {@link PieChart}. (Colors are used
	 * sequentially and then, circularly).
	 *
	 * @param colors
	 *            List of one or more colors.
	 */
	public void setColors(final AbstractColor... colors) {
		this.colors = colors.length == 0 ? null : colors;
	}

	/**
	 * Get the tooltip. (If <code>true</code> is passed as the parameter, a new tooltip will be created if not already
	 * exists).
	 *
	 * @param create
	 ...
	 * @return Tooltip.
	 */
	public Tooltip getTooltip(final boolean create) {
	 ...
	 * @return Label.
	 */
	public final Label getLabel(final boolean create) {
	...
	public Object getMin() {
		final AbstractDataProvider<?> d = data4MinMax();
	...
	private AbstractDataProvider<?> data4MinMax() {
		...
		for (final AbstractDataProvider<?> dp : data) {
	 ...
	 *
	 * @param create
	 *            If passed true, a new style is created if not exists.
	 * @return Item style.
	 */
	public final ItemStyle getItemStyle(final boolean create) {
	 ...
	 *
	 * @param create
	 *            If passed true, a new {@link MarkArea} is created if not exists.
	 * @return Mark area.
	 */
	public MarkArea getMarkArea(final boolean create) {
	 ...
	 *            Mark area to set.
	 */
	public void setMarkArea(final MarkArea markArea) {
		this.markArea = markArea;
	}

	@Override
	public Animation getAnimation(final boolean create) {
	 ...
	 *
	 * @param create
	 *            If passed true, a new {@link Emphasis} effect is created if not exists.
	 * @return Emphasis.
	 */
	@Override
	public Emphasis getEmphasis(final boolean create) {
	 ...
	 */
	@Override
	public void setEmphasis(final com.storedobject.chart.Emphasis emphasis) {
		if (emphasis instanceof Emphasis) {
			this.emphasis = (Emphasis) emphasis;
		}
	}
	...
	public static class Label extends com.storedobject.chart.Label {
		 ...
		 * <p>
		 * Format template may contain patterns like {n} where "n" is the index of the axis. So {0} represents the
		 * x-axis value and {1} represents the y-axis value for an {@link XYChart}. If 2 y-axes are there, it will be
		 * {1} and {2} respectively. There can be many axes and the index value will increase accordingly. The index
		 * indicates the order in which the axes were added to the chart.
		 * </p>
		 * <p>
		 * You could also use {D:data name} to specify any data set so that the labels will be taken from the data set
		 * specified. This mechanism is useful when you have a confusing set of axes and could not precisely determine
		 * the index number of the axis. Also, please see if you can use {@link #setLabelProvider(AbstractDataProvider)}
		 * instead of this.
		 * </p>
		 *
		 * <pre>
		...
		private String format(String f) {
			...
			final SOChart soChart = chart.soChart;
			if (soChart != null) {
				for (final AbstractDataProvider<?> data : soChart.dataSet()) {
		...
		/**
		 * Instead of specifying the format for the label via {@link #setFormatter(String)}, it is possible to set the
		 * labels directly from an {@link AbstractDataProvider} using this method. Such an {@link AbstractDataProvider}
		 * may be created from the one or more of the {@link AbstractDataProvider}s involved via one of the create
		 * methods: {@link AbstractDataProvider#create(DataType, Function)} or
		 * {@link AbstractDataProvider#create(DataType, BiFunction)}
		 *
		 * @param labels
		 *            Labels to be set from the elements of this data provider. (Typically a {@link CategoryData} is
		 *            used).
		 */
		public void setLabelProvider(final AbstractDataProvider<?> labels) {
			formatter = ""; // Set to a non-null value.
			this.labels = labels;
		}
	...
	public static class LabelPosition {
		 ...
		 *            Y position.
		 */
		public void at(final Size x, final Size y) {
			this.x = x;
			this.y = y;
		...
		private boolean set(final Size size) {
			switch (size.get()) {
			case -101:
				left = true;
				break;
		...
		private String encode(final boolean inside) {
			String s = toString();
			if (inside) {
				s = "inside" + s;
			}
	...
	public static class Emphasis extends com.storedobject.chart.Emphasis {
		...
		public enum FADE_OUT_SCOPE {
			...
			FADE_OUT_SCOPE(final String code) {
				this.code = code;
			}
		 ...
		 *            Fade out.
		 */
		public void setFadeOut(final FADE_OUT fadeOut) {
		 ...
		 *            Fade out scope.
		 */
		public void setFadeOutScope(final FADE_OUT_SCOPE fadeOutScope) {
			this.fadeOutScope = fadeOutScope;
		}

		@Override
		protected void encodeProperty(final StringBuilder sb) {
